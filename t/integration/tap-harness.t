...
describe 'a TAP::Parser::Aggregator' => sub {
  it "returns the number of failed tests when calling 'failed'" => sub {
    my $result = a_tap_harness()->runtests(A_FAILING_TEST);
    is $result->failed, 1;
  };
};

sub a_tap_harness { TAP::Harness->new }
